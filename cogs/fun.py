...
class Fun(commands.Cog):
    ...
    async def hug(self, ctx, member: discord.Member = None, intensity: int = 1):
        ...
        if intensity <= 0:
            msg = f"(っ˘̩╭╮˘̩)っ **{member.name}**"
        elif intensity <= 3:
            msg = f"(っ´▽｀)っ **{member.name}**"
        elif intensity <= 6:
            msg = f"╰(*´︶`*)╯ **{member.name}**"
        elif intensity <= 9:
            msg = f"(つ≧▽≦)つ **{member.name}**"
        else:
            msg = f"(づ￣ ³￣)づ **{member.name}** ⊂(´・ω・｀⊂)"
    ...
    async def pat(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(f"{random.choice(pats)} *pats* **{member.name}**")

    @commands.command()
    async def boop(self, ctx, member: discord.Member = None):
        """Boop someone's nose"""
        if member is None:
            await ctx.send("You cannot boop the void.. :(")
            return
        ...
        await ctx.send(f"**{member.name}** {random.choice(boops)}")
    ...
    async def slap(self, ctx, member: discord.Member = None):
        ...
        items = [
            ...
            "a soggy sandwich",
            "a slightly stale croissant",
            "a confused goose",
            "a feather duster",
            "a wobbly jello mold",
            "a tiny trombone",
            "a novelty foam finger",
            "a spilled latte",
            "a garden gnome",
            "a VHS tape full of infomercials",
            "a very small cactus in a teacup",
            "a children's picture book",
            "a sock puppet",
            "a pair of mismatched socks",
            "a suspiciously warm cookie",
            "the last slice of cake",
            "a kazoo solo",
            "a 'World's Okayest Human' mug",
            "a packet of expired coupons",
            "a 90s poster with questionable haircuts",
            "a rogue balloon animal",
            "a live rendition of a nursery rhyme",
            "a decorative pillow shaped like a cloud",
            "a slightly damp napkin",
            "a novelty singing fish",
            "an apologetic sticky note",
            "a tiny disco ball",
            "a collection of novelty stickers",
            "a miniature rubber boot",
            "a grocery list to nowhere",
    ...
    async def throw(self, ctx, member: discord.Member = None):
        ...
        items = [
            "a potato",
            "a rubber duck",
            "a bouquet of wilting lilies",
            "an ancient sandwich",
            "a pineapple pizza",
            "a suspiciously sticky jelly donut",
            "a parade of imaginary ants",
            "a bucket of confetti (cleanup required)",
            "a single sock with attitude",
            "a potted plant named Gerald",
            "a stack of unsolicited brochures",
            "a disgruntled plushie",
            "a glitter-filled pinata",
            "a decorative spoon collection",
            "a miniature garden gnome",
            "a half-eaten burrito",
            "a wet frisbee",
            "a bag of slightly stale chips",
            "a vintage alarm clock that only rings on Tuesdays",
            "a mysterious glowing rock (probably polished glass)",
            "a kazoo choir",
            "a broken umbrella that still tries",
            "a dozen mismatched buttons",
            "a signed napkin from a minor celebrity",
            "a crate of unopened takeout fortune cookies",
            "an enthusiastic opera singer (please excuse the noise)",
            "a tiny, indignant raccoon (containment required)",
            "a collection of novelty fridge magnets",
            "a thrift-store sweater with questionable patterns",
            "a handwritten poem about sandwiches",
            "a plastic tiara with one gem missing",
            "a map to nowhere",
    ...
    async def cookies(self, ctx, member: discord.Member = None):
        ...
        await ctx.send(f"**{member.name}** has **{cookie_count}** {cookie_text}! 🍪\nThey have eaten **{eaten_count}** {eaten_text} in total.")
    ...
    async def eat(self, ctx, amount: int = 1):
        ...
        # Validate amount
        if amount <= 0:
            await ctx.send("You can't eat a negative number of cookies!")
            return
    ...
    async def give(self, ctx, member: discord.Member, amount: int = 1):
        ...
        if amount <= 0:
            await ctx.send("You can't give a negative number of cookies!")
            return
    ...
    async def mock(self, ctx, *, text: Optional[str] = None):
        ...
        If no text is provided, mock the replied-to message or the last message
        sent before this command in the channel (mirrors `uwu` behaviour).
        """
        if text is None:
            if ctx.message.reference:
                ...
                try:
                    msgs = [m async for m in ctx.channel.history(limit=1, before=ctx.message.created_at)]
                    if msgs:
                        text = msgs[0].content
                    else:
                    ...
                    await ctx.send("Couldn't fetch recent messages!")
                    return
    ...
    @commands.command(aliases=['huh', 'how'])
    async def what(self, ctx):
        """Question life"""
        await ctx.send(f"I ask myself the same thing every day...")
