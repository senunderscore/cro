...
class Admin(commands.Cog):
    ...
    async def _set_log_channel(self, guild_id: int, setting_name: str, channel_id: Optional[int]) -> None:
        """Helper method to set a log channel setting.
        
        Args:
            guild_id: ID of the guild
            setting_name: Name of the setting
            channel_id: ID of the channel or None to clear
        """
        self.bot.settings.set_server_setting(guild_id, setting_name, channel_id)
    
    async def _get_log_channel(self, ctx: commands.Context, setting_name: str) -> Optional[discord.TextChannel]:
        """Helper method to get a log channel.
    ...
    async def config(self, ctx: commands.Context, setting: Optional[str] = None, *, value: Optional[str] = None) -> None:
        ...
        if not setting:
            ...
            for key, val in settings.items():
                if key.startswith('log_channel_'):
                    channel = ctx.guild.get_channel(val) if val else None
                    embed.add_field(
    ...
    async def _eval(self, ctx: commands.Context, *, code: str) -> None:
        """Evaluates Python code. Bot developers only."""
        env = {
            'ctx': ctx,
            'bot': self.bot,
            'channel': ctx.channel,
    ...
    async def description(self, ctx: commands.Context, *, description: Optional[str] = None) -> None:
        ...
        try:
            ...
            if len(description) > 1024:
                await ctx.send("Topic cannot exceed 1024 characters.")
                return
            ...
            logger.exception(f"Error setting channel description")
    ...
    @PermissionHandler.has_permissions(manage_channels=True)
    async def channelname(self, ctx: commands.Context, *, new_name: Optional[str] = None) -> None:
        ...
        try:
            ...
            if len(new_name) > 100:
                await ctx.send("Channel name cannot exceed 100 characters.")
                return
            ...
            logger.exception(f"Error setting channel name")
            await ctx.send(f"An error occurred: {str(e)}")
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def toggleprefix(self, ctx: commands.Context) -> None:
    ...
    async def tagcreate(self, ctx: commands.Context, name: str, *, content: str) -> None:
        """Create a new tag."""
        if len(name) > 50:
            return await ctx.send("Tag name cannot exceed 50 characters.")
        if len(content) > 2000:
            return await ctx.send("Tag content cannot exceed 2000 characters.")
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def joinleave(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None) -> None:
        ...
        Examples:
        - `{prefix}joinleave #welcome-logs`
        - `{prefix}jl #member-logs`
        - `{prefix}joinleave view`
        - `{prefix}joinleave disable`
        """
        if ctx.invoked_subcommand is None:
            if not channel:
                channel = ctx.channel
                
            await self._set_log_channel(ctx.guild.id, 'log_channel_join_leave', channel.id)
            await ctx.send(f"Join/Leave logs will be sent to {channel.mention}")

    @joinleave.command(name='view')
    @PermissionHandler.has_permissions(administrator=True)
    async def joinleave_view(self, ctx: commands.Context) -> None:
        """View current join/leave log channel."""
        channel = await self._get_log_channel(ctx, 'log_channel_join_leave')
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def joinleave_disable(self, ctx: commands.Context) -> None:
        """Disable join/leave logging."""
        await self._set_log_channel(ctx.guild.id, 'log_channel_join_leave', None)
        await ctx.send("Join/leave logging has been disabled")

    @commands.group(aliases=['ml'], invoke_without_command=True)
    @PermissionHandler.has_permissions(administrator=True)
    async def messagelogs(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None) -> None:
        ...
        Examples:
        - `{prefix}messagelogs #message-logs`
        - `{prefix}ml #logs`
        - `{prefix}messagelogs view`
        - `{prefix}messagelogs disable`
        """
        if ctx.invoked_subcommand is None:
            if not channel:
                channel = ctx.channel
                
            await self._set_log_channel(ctx.guild.id, 'log_channel_messages', channel.id)
            await ctx.send(f"Message logs (edits/deletions) will be sent to {channel.mention}")

    @messagelogs.command(name='view')
    @PermissionHandler.has_permissions(administrator=True)
    async def messagelogsview(self, ctx: commands.Context) -> None:
        """View current message logs channel."""
        channel = await self._get_log_channel(ctx, 'log_channel_messages')
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def messagelogsdisable(self, ctx: commands.Context) -> None:
        """Disable message logging."""
        await self._set_log_channel(ctx.guild.id, 'log_channel_messages', None)
        await ctx.send("Message logging has been disabled")

    @commands.group(aliases=['ma'], invoke_without_command=True)
    @PermissionHandler.has_permissions(administrator=True)
    async def modaudit(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None) -> None:
        ...
        Examples:
        - `{prefix}modaudit #mod-logs`
        - `{prefix}ma #audit-logs`
        - `{prefix}modaudit view`
        - `{prefix}modaudit disable`
        """
        if ctx.invoked_subcommand is None:
            if not channel:
                channel = ctx.channel
                
            await self._set_log_channel(ctx.guild.id, 'log_channel_mod_audit', channel.id)
            await ctx.send(f"Moderation logs will be sent to {channel.mention}")

    @modaudit.command(name='view')
    @PermissionHandler.has_permissions(administrator=True)
    async def modaudit_view(self, ctx: commands.Context) -> None:
        """View current mod audit log channel."""
        channel = await self._get_log_channel(ctx, 'log_channel_mod_audit')
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def modaudit_disable(self, ctx: commands.Context) -> None:
        """Disable mod audit logging."""
        await self._set_log_channel(ctx.guild.id, 'log_channel_mod_audit', None)
        await ctx.send("Mod audit logging has been disabled")

    @commands.group(aliases=['sb'], invoke_without_command=True)
    @PermissionHandler.has_permissions(administrator=True)
    async def starboard(self, ctx: commands.Context, channel: Optional[discord.TextChannel] = None) -> None:
        ...
        Examples:
        - `{prefix}starboard #starboard-channel`
        - `{prefix}sb #highlights`
        - `{prefix}starboard view`
        - `{prefix}starboard disable`
        - `{prefix}starboard limit 5`
        """
        if ctx.invoked_subcommand is None:
            if not channel:
                channel = ctx.channel
                
            await self._set_log_channel(ctx.guild.id, 'starboard_channel', channel.id)
            await ctx.send(f"Starboard messages will be sent to {channel.mention}")

    @starboard.command(name='limit')
    @PermissionHandler.has_permissions(administrator=True)
    async def starboard_limit(self, ctx: commands.Context, number: int) -> None:
        """Set the minimum number of stars needed."""
        if number < 1:
            await ctx.send("Limit must be at least 1")
            return
        if number > 100:
            await ctx.send("Limit cannot exceed 100")
            return
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def starboard_view(self, ctx: commands.Context) -> None:
        """View current starboard settings."""
        settings = self.bot.settings.get_all_server_settings(ctx.guild.id)
        channel = await self._get_log_channel(ctx, 'starboard_channel')
    ...
    @PermissionHandler.has_permissions(administrator=True)
    async def starboard_disable(self, ctx: commands.Context) -> None:
        """Disable the starboard."""
        await self._set_log_channel(ctx.guild.id, 'starboard_channel', None)
