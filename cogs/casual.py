...
class Casual(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description=(
                "Developed with 💜 by [Sen](https://discord.com/users/374589381752913930) as a hobby project.\n\n"
        ...
        embed.add_field(
            ...
            value=(
                ...
                "[Support](https://github.com/senunderscore/cro) • "
                "[GitHub](https://github.com/senunderscore/cro) • "
            ),
            inline=False
        )
    ...
    async def issues(self, ctx):
        ...
        await ctx.send("Click [this link](https://github.com/senunderscore/cro/issues) to report an issue or request a feature!")
    ...
    ## Server Info Command
    #################################
    @commands.command(aliases=['server', 'scan'])
    @commands.guild_only()
    async def serverinfo(self, ctx, guild: discord.Guild = None):
        """Display detailed server information.

        If `guild` is provided (ID or name), shows info for that server when
        the bot is present there. Alias: `server`, `scan`.
        """
        if guild is None:
            guild = ctx.guild
        ...
        basic = (
            f"Owner: {getattr(guild.owner, 'mention', str(guild.owner))}\n"
            f"ID: `{guild.id}`\n"
            f"Created: {discord.utils.format_dt(guild.created_at, 'F')} ({discord.utils.format_dt(guild.created_at, 'R')})"
        )
        ...
        channels = f"Text: **{text_channels}** • Voice: **{voice_channels}** • Categories: **{categories}**"
        roles = f"Roles: **{len(guild.roles)}** • Top role: {guild.roles[-1].mention if guild.roles else 'None'}"
        embed.add_field(name="Channels & Roles", value=f"{channels}\n{roles}", inline=False)

        extras = f"Emojis: **{emoji_count}**"
        if guild.premium_tier and guild.premium_tier > 0:
            extras += f" • Boosts: **{guild.premium_subscription_count}** (Level {guild.premium_tier})"
        ...
        embed.add_field(name="Security", value=f"Verification: {verification.get(guild.verification_level, str(guild.verification_level))}", inline=False)
